...
dayHostCount = dayGroupedHosts.map(lambda x:(x[0],len(list(x[1])))).sortByKey()
dailyHosts = (dayHostCount.cache())
dailyHostsList = dailyHosts.take(30)
print 'Unique hosts per day: %s' % dailyHostsList
#print ' %s ' % dayHostCount.take(30)
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda x:(x[0],(len(list(x[1]))/len(set(list(x[1])))))).cache())
...
badEndpointsTop20 = badEndpointsSum.map(lambda x:(x[1],x[0])).sortByKey(False).map(lambda x:(x[1],x[0])).take(20)
