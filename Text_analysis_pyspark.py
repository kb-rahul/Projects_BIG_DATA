...
def tokenize(string):
    ...
    """
    return [x for x in simpleTokenize(string) if x not in stopwords]#list(set(simpleTokenize(string))-set(stopwords))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda x:len(list(x[1]))).reduce(lambda a,b:a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda x:(x[0],(list(x[1])))).reduceByKey(lambda a,b:a+b).takeOrdered(1,lambda s:-1*len(s[1]))
...
def norm(a):
    ...
    """
    return sum([float(a[k]**2) for k in a.keys()])**0.5
...
avgSimNon = nonDupsRDD.filter(lambda x:x[1][1]==None).map(lambda x:(x[1][0])).reduce(lambda a,b:a+b)/float(nonDupsRDD.filter(lambda x:x[1][1]==None).count())
...
#print '%s' %fullCorpusRDD.take(3)
idfsFull = idfs(fullCorpusRDD)
idfsFullCount = idfsFull.count()
print 'There are %s unique tokens in the full datasets.' % idfsFullCount
#print '%s' %idfsFull.take(1)
# Recompute IDFs for full dataset

idfsFullWeights = idfsFull.reduceByKey(lambda a,b:a+b).map(lambda x:(x[0],x[1]/idfsFullCount)).collectAsMap()
#print idfsFullWeights.take(1)
idfsFullBroadcast = sc.broadcast(idfsFullWeights)
#print idfsFullBroadcast.value
...
#    newRec=amazonRec.catesian(googleRec)
#    for t in tokens:
#        s+=amazonWeightsBroadcast.value[amazonRec][t]*googleWeightsBroadcast.value[googleRec][t]
#    print s
#    s = [sum(amazonWeightsBroadcast.value[amazonRec][tok]*googleWeightsBroadcast.value[googleRec][tok]) for tok in tokens]
#    print s
#   value = s/[(sum(amazonWeightsBroadcast.value[amazonRec][tokens])*sum(googleWeightsBroadcast.value[googleRec][tokens])) for tok in tokens] 
...
#fastCosineSimilarity(commonTokens)
